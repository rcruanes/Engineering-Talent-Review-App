...
const pool = require('./comfig/db.js');
require('dorenv/config');

pool.getConnection( (err,conn) => {
  if (err) throw err;

  const firstname = 'raph'
  const lastname = 'pesquie'
  const department = 'CSE'
  const qry = 'INSERT INTO users (firstname,lastname,department) VALUES(?,?,?)';
  conn.query(qry, [firstname, lastname, department], (err, result) =>{
    conn.release();
    if(err) throw err;
    console.log(result);
  });
